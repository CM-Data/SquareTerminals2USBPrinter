...
showroom_employees = ['TMkOHwG2sxxltMhC', 'TMAjCxOpEODrZzdw', 'TMTg5jRjIGuWabZ4', 'TMweSIwsRqr-Yg3B' ]
...
def print_receipt_cups(receipt_text):
    ...
    # Print the receipt
    conn.printFile(default_printer, '/tmp/receipt.txt', "Receipt", {})

bearer_auth_credential = BearerAuthCredentials(
access_token = 'EAAAlxNHS-lVdvkyGgMiAT5kruyz0C5-VNMFPBeyXQfjehRF0v8H2uVuXKG2NCk_')

client = Client(
    bearer_auth_credentials=bearer_auth_credential,
    environment='production'  # Change to 'sandbox' if you're using the sandbox environment
)
    ...
    try:
        ...
        if response.is_success():
            ...
            for item in line_items:
                ...
                item_note = line_item.get('note', 'No note for this item')
...
app = Flask(__name__)
run_with_ngrok(app=app, auth_token='2lcXPEMKY82Tlmr0zKlrUUXSmtV_2sVxqLBAXfA9FD64vvcLG')
...
def webhook():
    if request.method == 'POST':
        ...
        if employee_id in showroom_employees:
